# My MIT-6.824
## 前言
目前 raft能够通过所有测试用例，但偶尔还会有bug产生，将会在后续（已经连肝半个多月了，后续几天进展缓慢，再加上科研与学业压力，等到寒假再集中精力攻克）再做出下列改进，改进bugfree后，补充各个模块的具体实现（目前版本代码已上传）。
todo: 将time.Timer的计时器触发更改为time与sleep混合使用的触发模式。
## lab1：MapReduce
### 任务说明
