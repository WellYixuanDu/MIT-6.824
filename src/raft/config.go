...
// 将cmd指令加入到leader中，并在2s内判断leader是否能够将leader发送给指定书目的服务器中，若 retry为 false则只在2s内判断，为true则可以重复尝试判断
func (cfg *config) one(cmd interface{}, expectedServers int, retry bool) int {
	t0 := time.Now()
	starts := 0
	for time.Since(t0).Seconds() < 10 && cfg.checkFinished() == false {
		// try all the servers, maybe one is the leader.
		// 找到leader的index,并将cmd命令写入到leader的log中，返回该命令的log下标为index
		index := -1
		for si := 0; si < cfg.n; si++ {
			starts = (starts + 1) % cfg.n
			var rf *Raft
			cfg.mu.Lock()
			// starts 索引下的net是否在网络上
		...
		if index != -1 {
			// 2s内是否能让expectedServers个服务器在该索引下获得log
			t1 := time.Now()
			//println("begin")
			for time.Since(t1).Seconds() < 2 {
				TPrintf("the index is %v", index)
				nd, cmd1 := cfg.nCommitted(index)
				TPrintf("The leader's info is %v, cmd is %v, cmd1 is %v", nd, cmd, cmd1)
				// 如果expectedServes个raft在该索引下均存在该log，则返回该index
				if nd > 0 && nd >= expectedServers {

					// committed
					if cmd1 == cmd {
						// and it was the command we submitted.
						TPrintf("%v win the test", index)
						return index
					}
				}
