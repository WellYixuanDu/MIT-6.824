...
func TestRPCBytes2B(t *testing.T) {
	...
	// 将99加入到leader中，并在2s内判断leader是否能够将leader发送给指定书目的服务器中，若 retry为 false则只在2s内判断，为true则可以重复尝试判断
	cfg.one(99, servers, false)
	bytes0 := cfg.bytesTotal()
	TPrintf("The bytes0 is %v", bytes0)
...
func TestFailAgree2B(t *testing.T) {
	...
	TPrintf("%v is the leader and disconnect a follower from the internet", leader)
	...
	TPrintf("--------------------------Test (2B): agreement after follower reconnects pass-----------------------------")
	cfg.end()
}
...
func TestFailNoAgree2B(t *testing.T) {
	...
	TPrintf("--------------------------Test (2B): no agreement if too many followers disconnect pass-----------------------------")
	time.Sleep(3)
	cfg.end()
}
...
loop:
	...
	TPrintf("--------------------------Test (2B): concurrent Start()s pass-----------------------------")
	time.Sleep(3)
	cfg.end()
}
...
func TestRejoin2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	cfg.disconnect(leader1)
	TPrintf("%v leader is disconnected", leader1)
	...
	cfg.connect(leader1)
	TPrintf("%v connect again", leader1)
	cfg.one(104, 2, true)
	// leader 失联后，再连接上时状态如何进行转化
	// all together now
	cfg.connect(leader2)
...
func TestPersist12C(t *testing.T) {
	...
	TPrintf("The second log will be added")
...
func TestFigure8Unreliable2C(t *testing.T) {
	...
	TPrintf("--------------------------FIGURE 8 UNREALIBLE PASS-------------------------")
	cfg.end()
}
...
func snapcommon(t *testing.T, name string, disconnect bool, reliable bool, crash bool) {
	...
	for i := 0; i < iters; i++ {
		...
		if disconnect {
			...
			TPrintf("**********************************The disconnect node is %v*****************************************", victim)
		...
		if disconnect {
			...
			cfg.connect(victim)
			TPrintf("********************************** The node %v is connected, the test is %v *****************************************", victim, iters)
			cfg.one(rand.Int(), servers, true)
			leader1 = cfg.checkOneLeader()
		}
		if crash {
